=== MarcTV Galleria  ===
Contributors: MarcDK
Tags: MarcTV, jquery, galleria, gallery, responsive, fullscreen, twentyfifteen, google analytics
Requires at least: 3.0
Tested up to: 4.4.2
Stable tag: 2.7

== Description ==

A neat and simple sliding image gallery. Based on the mighty
Galleria http://galleria.io/ It comes with responsiveness, fullscreen mode and touch events.

Configurable image size for custom break point.

== Installation ==

* Install plugin
* Activate it
* Configure under settings

== Changelog ==

= 2.7 =

* Added Google Analytics (Universal) event support.

= 2.6.7 =

* Removed unnecessary img attributes.

= 2.6.6 =

* Optimized image assets.
* Works in IE again. Forgot to remove a console.debug

= 2.6.4 =

* Fixed thumbnails sizes.
* Loads galleria minimized.

= 2.6.3 =

* Firefox background fix.
* Optimized image sizes.

= 2.6.2 =

* Better loader.

= 2.6.1 =

* Fixed typo and moved scripts to load in the footer.

= 2.6 =

* Fixed fullscreen counter color.
* Added option to select fullscreen image size after window breakpoint.
* Added screenshots.

= 2.5.1 =

* Image size for window size breakpoint can be configured in the backend.
* Choose an image size for gallery for this breakpoint.
* Rewritten in OO PHP
* New meta information.
* Compatible with Tweenty Fifteen.
* Localisation is now supported. Feel free to send me your translation.

= 2.3 =

New Throbber position. 

= 2.2 =

Fixed CSS bug in thumb navigation.

= 2.1 =

* Updated to Galleria 1.4.2
* Gallery now always links to the media file and not the media page. 

= 2.0.6 =

* Fixed size on devices with less than 800px. (no-thumbnail mode)

= 2.0.4 =

* Fixed fullscreen on iOS Devices.

= 2.0.3 =

Added retina sprite.

= 2.0.2 =

 * fullscreen issues fixed on iPad.

= 2.0.1 =

 * font-style fixes.

= 2.0 =

Big changes!

* Updated Galleria to 1.3.6
* MarcTV Galleria Theme is now based on the classic theme of galleria. Only css and
init js changes. Leaves original and solid working code working.
* Thumbnails for devices greater than 800px.
* New image style for thumbnails.
* Real retina sprites. No SVG Sprites anymore.
* New black fullscreen theme.
* No Google Analytics Events anymore.

= 1.8.5 =

Updated Galleria to 1.3.5

= 1.8.2 =

Updated Galleria to 1.3.2

= 1.8 =

Updated Galleria to 1.3.1 with touch events and fullscreen fix.

= 1.7.4 =

* Fixed position of all sprites.
* Galleria 1.2.9 update

= 1.7.2 =

* Added CSS to reflect the missing title attribute in WP 3.5

= 1.7 =

* disabled doubleclick for fullscreen on iOS

= 1.6 =

* SVG 1.1 support for compatible browsers

= 1.5 =

* fixed position of info button
* Added fix for IE8

= 1.4 =

Prefix added as namespace

= 1.3 =

Idle mode fixed

= 1.2 =

Responsive bug fixed

= 1.1 =

Initial release


== Screenshots ==

1. The plugin in action.
2. The gallery in fullscreen mode.
3. The settings page.